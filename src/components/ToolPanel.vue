<template>
  <section class="tool-panel" :hidden="!toolbarVisible">
    <h3>Tool</h3>
    <button type="button" @click="toolSelected = 'eraser'"
      :class="{ 'use-tool': toolSelected === 'eraser' }">Eraser</button>
    <button type="button" @click="toolSelected = 'drag'" :class="{ 'use-tool': toolSelected === 'drag' }">Drag</button>
    <button type="button" @click="toolSelected = 'drag-frame'"
      :class="{ 'use-tool': toolSelected === 'drag-frame' }">Drag frame</button>
    <button type="button" @click="toolSelected = 'pencil'"
      :class="{ 'use-tool': toolSelected === 'pencil' }">Pencil</button>
  </section>
  <section class="tool-panel" v-if="toolSelected === 'eraser'">
    <h3>Eraser Settings</h3>
    <label for="eraserSize">Eraser size</label>
    <input type="number" id="eraserSize" v-model="eraserSize" step="5" min="0" max="1000" />
  </section>
...
import { eraserSize, snapSize, toolSelected, pencilColor, toolbarVisible } from '@/services/editorAppState';
