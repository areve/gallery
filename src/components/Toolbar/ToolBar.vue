<template>
  <AppWindow title="Toolbar" v-model:panelState="toolbarState.panel">
    <section class="tool-panel">
      <h3>Tool</h3>
      <button type="button" @click="toolbarState.toolSelected = 'eraser'" :class="{ 'use-tool': toolbarState.toolSelected === 'eraser' }">Eraser</button>
      <button type="button" @click="toolbarState.toolSelected = 'drag'" :class="{ 'use-tool': toolbarState.toolSelected === 'drag' }">Drag</button>
      <button type="button" @click="toolbarState.toolSelected = 'drag-frame'" :class="{ 'use-tool': toolbarState.toolSelected === 'drag-frame' }">
        Drag frame
      </button>
      <button type="button" @click="toolbarState.toolSelected = 'pencil'" :class="{ 'use-tool': toolbarState.toolSelected === 'pencil' }">Pencil</button>
    </section>
    <section class="tool-panel" v-if="toolbarState.toolSelected === 'eraser'">
      ...
      <input type="number" id="eraserSize" v-model="eraserToolState.eraserSize" step="5" min="0" max="1000" />
    </section>
    <section class="tool-panel" v-if="toolbarState.toolSelected === 'drag' || toolbarState.toolSelected === 'drag-frame'">
      <h3>Drag Settings</h3>
      <label for="snap">snap</label>
      <input type="number" id="snap" v-model="dragToolState.snapSize" step="1" min="1" max="256" />
    </section>
